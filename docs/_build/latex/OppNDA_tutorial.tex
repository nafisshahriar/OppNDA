...
\usepackage[utf8]{inputenc}
\usepackage{minted}
\usepackage{tcolorbox} % For the "Example" box effect
\tcbuselibrary{skins, breakable}

% Define a simple box for the "Example" note
\newtcolorbox{myexample}[1]{
  colback=blue!5, colframe=blue!75!black, fonttitle=\bfseries,
  title=#1, arc=0mm, breakable
}
...
OppNDA (Opportunistic Network Data Analyzer) is a comprehensive framework that streamlines the workflow for configuring, running, and analyzing simulations in the ONE (Opportunistic Network Environment) simulator. This document provides a step-by-step walkthrough of the framework, supplementing the main manuscript titled \textbf{OppNDA: A Modular and Scalable Automation Framework for Streamlining DTN Research with the ONE Simulator}.
...
\begin{enumerate}
    ...
    \item \textbf{Averaging}: Aggregate results across multiple simulation seeds
    \item \textbf{Analysis}: Generate publication-ready visualizations
    \item \textbf{Regression}: Train machine learning models on simulation data
\end{enumerate}

\screenshotplaceholder{workflow-diagram}{OppNDA workflow pipeline overview}
...
git clone https://github.com/nafisshahriar/oppnda.git
cd oppnda
    \end{lstlisting}
...
\screenshotplaceholder{setup-windows}{Running setup.bat on Windows}
...
\screenshotplaceholder{docker-running}{OppNDA running in a Docker container}

\begin{tipbox}
Docker is particularly useful for consistent deployment across different machines and for isolated testing environments. The containerized version includes all dependencies pre-configured.
\end{tipbox}
...
\screenshotplaceholder{homepage}{OppNDA web interface home page}
...
The Settings page provides a comprehensive interface for configuring ONE simulator scenarios. This section covers both creating new configurations and importing existing ones.
...
\screenshotplaceholder{import-config}{Importing an existing ONE configuration file}
...
\screenshotplaceholder{scenario-settings}{Basic scenario configuration panel}
...
\screenshotplaceholder{interface-config}{Network interface configuration}
...
\screenshotplaceholder{group-config}{Host group configuration panel}
...
\screenshotplaceholder{event-config}{Message event generator configuration}
...
\screenshotplaceholder{report-selection}{Report type selection panel}
...
\screenshotplaceholder{run-one-button}{The Run ONE button and configuration summary}
...
\screenshotplaceholder{console-output}{Real-time console output during simulation}
...
OppNDA uses configurable filename patterns to group and average reports. A typical ONE report filename follows this structure:
...
\subsection{Configuring the Averager}
...
\screenshotplaceholder{averager-config}{Averager configuration panel with pattern builder}
...
\screenshotplaceholder{pattern-builder}{Interactive pattern builder interface}

\subsection{Running the Averager}

\begin{enumerate}
    \item Click \textbf{Run Averager}
...
\screenshotplaceholder{averager-output}{Averager console output showing grouped files}
...
\screenshotplaceholder{analysis-config}{Analysis configuration panel}
...
\screenshotplaceholder{plot-types}{Plot type selection with preview}
...
\screenshotplaceholder{plot-settings}{Plot customization settings}
...
\screenshotplaceholder{analysis-running}{Analysis progress with real-time logging}
...
\screenshotplaceholder{generated-3d-plot}{Example 3D surface plot showing delivery ratio}

\screenshotplaceholder{generated-heatmap}{Example heatmap visualization}
...
\screenshotplaceholder{regression-config}{Regression configuration panel}

\subsection{Available ML Models}

OppNDA supports multiple regression algorithms:

\begin{itemize}
    \item \textbf{Linear Regression}: Simple interpretable model
    \item \textbf{Ridge Regression}: L2 regularized linear model
    \item \textbf{Lasso Regression}: L1 regularized for feature selection
    \item \textbf{Random Forest}: Ensemble tree-based method
    \item \textbf{Gradient Boosting}: Sequential ensemble method
    \item \textbf{Support Vector Regression}: Kernel-based method
\end{itemize}

\subsection{Multi-Target Regression}

OppNDA supports training models on multiple target variables simultaneously:

\begin{enumerate}
    \item Select multiple target variables from the dropdown
    \item Models are trained independently for each target
    \item Results are aggregated for comparison
\end{enumerate}
...
\begin{enumerate}
    ...
    \item View model performance metrics (R², RMSE, MAE)
\end{enumerate}

\screenshotplaceholder{regression-running}{Regression training progress}

\subsection{Interpreting Results}

The regression results panel displays:

\begin{itemize}
    \item Model performance comparison table
    \item Feature importance rankings
    \item Actual vs. predicted scatter plots
    \item Cross-validation scores
\end{itemize}

\screenshotplaceholder{regression-results}{Regression results and model comparison}

\begin{tipbox}
Use feature importance to identify which simulation parameters have the greatest impact on network performance metrics. This can guide future simulation parameter selection.
\end{tipbox}
...
This tutorial has covered the complete OppNDA workflow from installation through regression analysis. Key takeaways:

\begin{enumerate}
    \item \textbf{Flexible Installation}: Choose between scripts, manual setup, or Docker
    \item \textbf{Intuitive Configuration}: Web-based GUI simplifies ONE simulator setup
    \item \textbf{Config Import}: Easily import and modify existing configuration files
    \item \textbf{Integrated Execution}: Run simulations and monitor in real-time
    \item \textbf{Automated Post-Processing}: Seamless averaging and visualization
    \item \textbf{Advanced Analytics}: ML-based regression for performance insights
\end{enumerate}

\subsection{Tips for Advanced Usage}

\begin{itemize}
    \item Use the performance optimization settings to handle large datasets
    \item Leverage the dynamic memory management for parallel processing
    \item Customize plot styles through the analysis configuration JSON
    \item Export configurations for reproducible experiments
\end{itemize}

\subsection{Further Resources}

\begin{itemize}
    \item OppNDA Documentation: \url{https://oppnda.readthedocs.io}
    \item ONE Simulator: \url{https://github.com/akeranen/the-one}
    \item GitHub Repository: \url{https://github.com/nafisshahriar/oppnda}
\end{itemize}
...
\begin{center}
    ...
    {\small DHMAI Network Research Group}\\
    {\small \url{https://dhmairg.net}}\\
    {\small 2026}
\end{center}
